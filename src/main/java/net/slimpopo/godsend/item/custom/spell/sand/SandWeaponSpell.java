...
public class SandWeaponSpell extends SpellItem {
    public static final Spell SANDWEAPONSPELL = new Spell("Sand Weapon Spell",15,3,
            "A spell that allows you to conjure the sand hammer.");
    public static int weaponMode = 0;

    public SandWeaponSpell(Properties pProperties) {
        super(pProperties, SANDWEAPONSPELL);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ItemStack HAMMER = new ItemStack(ModItems.SAND_HAMMER.get());
            ...
            if(mCur >= spell.getManaCost()) {
                ...
                if (availableSlot == -1) {
                    ...
                    if (weaponSlot == -1) {
                        ...
                        setWeaponToSlot(pPlayer, HAMMER);
                        weaponMode = 1;


                        ManaManager.get(pPlayer.level).loseMana(mCur - SANDWEAPONSPELL.getManaCost());
    ...
    public int checkWeaponMode(Player player){
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.SAND_HAMMER.get()){
                return 1;
            }
        }
        return 0;
    ...
    public int findSpellSlot(Player player){
        int slotNum = 0;
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.SANDSPELL_WEAPONIZE.get()){
                return slotNum;
            }
            slotNum++;
        }
