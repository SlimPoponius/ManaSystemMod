...
public class ShadowImbueSpell extends SpellItem {
    private static final Spell SHADOWIMBUESPELL = new Spell("Shadow Imbue Spell",5,1,
            "Imbue your weapon with the power of the shadows.");

    public ShadowImbueSpell(Properties pProperties) {
        super(pProperties, SHADOWIMBUESPELL);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if(!pLevel.isClientSide){
            ...
            if(mCur >= SHADOWIMBUESPELL.getManaCost()){
                if(pPlayer.getMainHandItem().getItem() == this){
                    ItemStack checkWeapon = pPlayer.getOffhandItem();
                    if(checkIfWeapon(checkWeapon)){
                        checkWeapon.enchant(ModEnchantments.SHADOWIMBUE.get(),1);
                        ManaManager.get(pPlayer.level).loseMana(mCur - SHADOWIMBUESPELL.getManaCost());
                    }
                }
