...
public class ThunderWeaponSpell extends SpellItem {
    public static final Spell THUNDERWEAPONSPELL = new Spell("Thunder Weapon Spell",20,5,
    ...
    public ThunderWeaponSpell(Properties pProperties) {
        super(pProperties, THUNDERWEAPONSPELL);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ItemStack RAPIER = new ItemStack(ModItems.THUNDER_RAPIER.get());
        ItemStack STAFF = new ItemStack(ModItems.THUNDER_STAFF.get());
            ...
            if(mCur >= spell.getManaCost()) {
                ...
                if (availableSlot == -1) {
                    ...
                    if (weaponSlot == -1) {
                        ...
                        setWeaponToSlot(pPlayer, RAPIER);
                        weaponMode = 1;


                        ManaManager.get(pPlayer.level).loseMana(mCur - THUNDERWEAPONSPELL.getManaCost());
                    ...
                    //WeaponType is found
                        ...
                        if (weaponMode == 1) {
                            replaceWeaponToSlot(pPlayer, STAFF);
                            weaponMode = 2;
                            ManaManager.get(pPlayer.level).loseMana(mCur - THUNDERWEAPONSPELL.getManaCost());
    ...
    public int checkWeaponMode(Player player){
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.THUNDER_RAPIER.get()){
                return 1;
            }
            else if(item.getItem() == ModItems.THUNDER_STAFF.get()){
                return 2;
            }
        }
        return 0;
    ...
    public int findSpellSlot(Player player){
        int slotNum = 0;
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.THUNDERSPELL_WEAPONIZE.get()){
                return slotNum;
            }
            slotNum++;
        }
    ...
    public int findWeaponType(Player player){
        int slotNum = 0;
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.THUNDER_RAPIER.get() || item.getItem() == ModItems.THUNDER_STAFF.get()){
                return slotNum;
            }
            slotNum++;
        }
    ...
    public void replaceWeaponToSlot(Player player, ItemStack itemPlace){
        ItemStack GREATSWD = new ItemStack(ModItems.THUNDER_RAPIER.get());
        ItemStack BOW = new ItemStack(ModItems.THUNDER_STAFF .get());
