...
public class EarthWeaponSpell extends SpellItem {
    ...
    public EarthWeaponSpell(Properties pProperties) {
        super(pProperties, EARTHWEAPONSPELL);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ItemStack AXE = new ItemStack(ModItems.EARTH_AXE.get());
        ItemStack HAMMER = new ItemStack(ModItems.EARTH_HAMMER.get());
            ...
            if(mCur >= spell.getManaCost()) {
                ...
                if (availableSlot == -1) {
                    ...
                    if (weaponSlot == -1) {
                        ...
                        setWeaponToSlot(pPlayer, AXE);
                        weaponMode = 1;


                        ManaManager.get(pPlayer.level).loseMana(mCur - EARTHWEAPONSPELL.getManaCost());
                    ...
                    //WeaponType is found
                        ...
                        if (weaponMode == 1) {
                            replaceWeaponToSlot(pPlayer, HAMMER);
                            weaponMode = 2;
                            ManaManager.get(pPlayer.level).loseMana(mCur - EARTHWEAPONSPELL.getManaCost());
    ...
    public int checkWeaponMode(Player player){
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.EARTH_AXE.get()){
                return 1;
            }
            else if(item.getItem() == ModItems.EARTH_HAMMER.get()){
                return 2;
            }
        }
        return 0;
    ...
    public int findSpellSlot(Player player){
        int slotNum = 0;
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.EARTHSPELL_WEAPONIZE.get()){
                return slotNum;
            }
            slotNum++;
        }
    ...
    public int findWeaponType(Player player){
        int slotNum = 0;
        for(ItemStack item: player.getInventory().items){
            if(item.getItem() == ModItems.EARTH_AXE.get() || item.getItem() == ModItems.EARTH_HAMMER.get()){
                return slotNum;
            }
            slotNum++;
        }
