...
public class LifeStealWeaponSpell extends SpellItem {
    private static final Spell LIFESTEALSPELL = new Spell("Life Steal Spell",5,1,
            "Imbue your weapon with the power to steal health.");

    public LifeStealWeaponSpell(Properties pProperties) {
        super(pProperties, LIFESTEALSPELL);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if(!pLevel.isClientSide){
            ...
            if(mCur >= LIFESTEALSPELL.getManaCost()){
                if(pPlayer.getMainHandItem().getItem() == this){
                    ItemStack checkWeapon = pPlayer.getOffhandItem();
                    if(checkIfWeapon(checkWeapon)){
                        checkWeapon.enchant(ModEnchantments.LIFESTEAL.get(),1);
                        ManaManager.get(pPlayer.level).loseMana(mCur - LIFESTEALSPELL.getManaCost());
                    }
                }
